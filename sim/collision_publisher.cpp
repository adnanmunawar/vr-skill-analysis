...
void DrillingPublisher::init(string a_namespace, string a_plugin){
    ...
    //Added
    m_forcePub = m_rosNode -> advertise<geometry_msgs::WrenchStamped>(a_namespace + "/" + a_plugin + "/force",1, true); 
}
...
// Added
void DrillingPublisher::force(double f[3], double time){
    force_msg.header.stamp.fromSec(time);

    force_msg.wrench.force.x = f[0];
    force_msg.wrench.force.y = f[1];
    force_msg.wrench.force.z = f[2];

    m_forcePub.publish(force_msg);
}
